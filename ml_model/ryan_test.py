...
def convert_to_number(uci_repr):
  first, second, third, fourth = uci_repr[:4]
  first = " abcdefgh".index(first)
  second = 10 * (10 - int(second))
  third = " abcdefgh".index(third)
  fourth = 10 * (10 - int(fourth))
  return (first + second, third + fourth)


def convert_to_uci(sunchess_square: int):
  tens_digit, ones_digit = sunchess_square // 10, sunchess_square % 10
  row = (10 - tens_digit)
  col = "abcdefgh"[ones_digit - 1]
  return f"{col}{row}"
...
class Searcher:
  ...
  def bound(self, pos, gamma, depth, root=True):
    ...
    entry = self.tp_score.get((pos, depth, root),
                              Entry(-MATE_UPPER, MATE_UPPER))
    if entry.lower >= gamma and (not root
                                 or self.tp_move.get(pos) is not None):
      return entry.lower
...
def eval(net: badgyal.AbstractNet, board: chess.Board, model_name: str):
  ...
  policy, value = net.eval(board, softmax_temp=1, name=model_name)
  # print(value)
...
def main():
  ...
  while not board.is_game_over():
    ...
    # human_move = get_legal_move(legal_moves)
    ...
    # # --- Add sunfish move to Leela hist ---
    # print(move)
    # uci_move = convert_to_uci(move[0]) + convert_to_uci(move[1])
    # print(uci_move)
    ...
    # --- Leela moves ---
    print_leela_chessboard(board)
    leela_move, leela_est_value = eval(model, board, model_name)
    print(f"Leela moves: {leela_move}\n")
    board.push_uci(leela_move)

    # --- Add Leela move to sunfish hist ---
    # sunfish_move = convert_to_number(leela_move)
    # print(f"Converting leela move from {leela_move} to sunfish move {sunfish_move}...")
    # hist.append(hist[-1].move(sunfish_move))
    # print_pos(hist[-1])
    # print()
